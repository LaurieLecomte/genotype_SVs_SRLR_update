...
# manitou
# srun -c 1 -p small -J 01_prepare_VCF_all_callers --mem=10G -o log/01_prepare_VCF_all_callers_%j.log /bin/sh ./01_scripts/01_prepare_VCF_all_callers.sh &
...
# 3. Extract insertion sequences
bcftools filter -i 'SVTYPE=="INS"' $VCF_DIR/candidates/"$(basename -s .ready.vcf $INPUT_VCF)".candidates.vcf | bcftools query -f ">%ID\n%ALT\n" > $VCF_DIR/candidates/"$(basename -s .ready.vcf $INPUT_VCF)"_INS.fa
